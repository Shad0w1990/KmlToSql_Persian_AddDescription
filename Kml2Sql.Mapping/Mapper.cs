...
namespace Kml2Sql.Mapping
{
    ...
    public class Kml2SqlMapper
    {
        ...
        public string GetCreateTableScript()
        {
            ...
            sb.Append($"[{Configuration.NameColumnName}] NVARCHAR(255),");
            sb.Append($"[{Configuration.DescriptionColumnName}] NVARCHAR(255),");
            foreach (var columnName in GetColumnNames().Select(Configuration.GetColumnName))
            {
                sb.Append(String.Format("[{0}] NVARCHAR(max), ", columnName));
            }
